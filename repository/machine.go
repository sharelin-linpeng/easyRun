...
func (machineService) QueryList() ([]entity.Machine, error) {
	sqlStr := "SELECT id,name, ip, login_name,password,env FROM machine"
...
func (machineService) QueryById(id string) (*entity.Machine, error) {
	sqlStr := "SELECT id,name, ip, login_name,password,env FROM machine where id = ?"
...
func (machineService) Add(machine entity.Machine) error {
	sqlStr := "INSERT INTO machine(name, ip, login_name,password,env) VALUE( ?, ?, ?, ?, ?)"
	if _, err := db.DB.Exec(sqlStr, machine.Name, machine.Ip, machine.LoginName, machine.Password, machine.Env); err != nil {
...
func (machineService) Update(machine entity.Machine) error {
	sqlStr := "UPDATE  machine SET name = ?, ip = ? , login_name = ? , password = ? ,env = ?  WHERE id = ?"
	if _, err := db.DB.Exec(sqlStr, machine.Name, machine.Ip, machine.LoginName, machine.Password, machine.Env, machine.Id); err != nil {
